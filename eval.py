...
def compute_timestamp_mae(
    ...
    """
    Word-level timestamp MAE с последовательным матчингом.

    Ключевой инсайт: и ref строки, и pred слова идут в одном
    временном порядке. Поэтому ищем каждое следующее слово
    только ПОСЛЕ позиции предыдущего матча (монотонность).

    Без этого короткие слова ("man", "all", "i") матчатся
    к первому вхождению в треке и ломают всю статистику.
    """
    if not ref_lines or not pred_words:
        return None

    pred_word_norms = [normalize(w["word"]) for w in pred_words]
    errors = []
    search_from = 0  # монотонный указатель — не идём назад

    for ref_line in ref_lines:
        # Берём первые N значимых слов строки для надёжного матча
        ref_tokens = [w for w in normalize(ref_line["text"]).split() if len(w) > 2]
        if not ref_tokens:
            continue
        # Якорь — первые 2 слова строки (снижает вероятность ложного матча)
        anchor = " ".join(ref_tokens[:2]) if len(ref_tokens) >= 2 else ref_tokens[0]

        best_score = 0.0
        best_idx = -1
        # Ищем только вперёд от search_from, с небольшим lookahead окном
        lookahead = min(search_from + 80, len(pred_word_norms))
        for i in range(search_from, lookahead):
            # Окно из 2 слов в pred для сравнения с anchor
            window = " ".join(pred_word_norms[i:i+2])
        ...
        if best_idx >= 0 and best_score >= min_similarity:
            errors.append(abs(pred_words[best_idx]["start"] - ref_line["time"]))
            search_from = best_idx + 1  # следующий матч только после этой позиции
